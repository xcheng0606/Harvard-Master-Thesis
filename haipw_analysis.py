...
N_SPLITS      = 30   
N_REPS        = 1000
SAMPLE_SIZES  = [100, 200, 344, 688, 1376]
SEED          = 42
...
# 14 harmonized covariates shared with IST for EXTERNAL models
SHARED_FEATURES = [
...
# Influence functions: φ₁ estimates E[Y(1)], φ₀ estimates E[Y(0)]
...
def crossfit_arm_models(Y, A, X, n_splits=N_SPLITS):
    """Fit arm-specific logistic models via K-fold cross-fitting. Returns Q1_hat, Q0_hat."""
    n = len(Y)
    Q1_hat = np.zeros(n)
    Q0_hat = np.zeros(n)
...
# H-AIPW: optimal combination across estimators per arm

def haipw_arm(phi_list):
    """Combine influence functions via optimal λ = Σ⁻¹1/(1ᵀΣ⁻¹1). Returns (θ̂ₐ, scaled_var, λ)."""
...
def run_subsampling(ist3_df, m_asp, ref_dm=None):
    """Random subsampling simulation. ref_dm is the complete-case DM used as reference for MSE/coverage."""
    if ref_dm is None:
...
def run_baseline(ist3_df, m_asp):
    """Single estimate on the complete-case IST3 sample (n=2752). Returns DM as coverage/MSE reference."""
    Y  = ist3_df["Y"].values
    A  = ist3_df["A"].values
